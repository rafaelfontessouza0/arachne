"""Crawl orchestration: static async phase + optional browser render phase."""
...
class Crawler:
    ...
    async def _enqueue_endpoint(self, raw: str, base: Optional[str], depth: int,
        ...
        for url in cands:
            ...
            if self.scope.in_scope(url):
                ...
                # record the discovery even though we will not fetch it
                self.out.write(Result(url=url, source=source, depth=depth, referrer=referrer,
                                      is_api=extract.looks_like_api(url),
                                      params=extract.param_names(url), note="out-of-scope"))
    ...
    async def _process(self, task: Task, resp) -> None:
        ...
        if is_html and "html" in ct:
            ...
            for raw in extract.extract_js(body.decode("utf-8", "replace")):
                await self._enqueue_endpoint(raw, final_url, task.depth + 1, final_url, "js")
        elif is_json:
            result.is_api = True
    ...
    async def _render_phase(self) -> None:
        ...
        try:
            ...
            self._log(f"[!] {exc}")
            return
        # fetch anything new the render surfaced
        await self._drain()
    ...
    async def run(self) -> dict:
        ...
        self._log(f"[+] done | fetched {self._fetched} | urls {summary['unique_urls']} "
                  f"| api {summary['api_endpoints']} | js {summary['js_files']} "
